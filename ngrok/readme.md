...
So why are we using ngrok? The addresses of Ethereum validator nodes in a peer-to-peer network using PoA (proof of authority) need to be specified in our bootnodes.json file, which is the file that tells other nodes how to reach validators. Basically, we have to know how to reach the validator nodes before we even start any of the nodes on our network. 

Usually, to give every node this contact information, we need to define the IP addresses of the validator nodes in bootnodes.json. Now when you are a student that's frequently between school, home and other places, your public IP address will almost never be the same. It will be assigned by the current internet you are using. So how do we account for the changes in IP addresses?
...
Now what this means for our network is that we can define several static domains as the addresses to which any machine can contact the validator nodes in our bootnodes.json file without using any IP addresses. And wherever the machines that are validators go, they'll still be contacted by others. 
...
So every time we want to start our nodes, we have to start ngrok on all validator nodes ahead of time and put their domains in our bootnodes.json file, which is the current configuration we are going to use. The instructions below give a short demo on how to do just that.
...
5. Take the domain and paste it into our bootnodes.json in the following format:
```json// {
"bootnodes" : [
    "enode://<your-machine's-public-key-here>@<your-static-domain-here>",
    "enode://<another-machine's-public-key-here>@<that-machine's-static-domain>",
    ...
]
}
```

6. Start the node once we have all of our ngrok sessions and machines running with the appropriate values in both our genesis.json and bootnodes.json file:
```command-line//
geth --datadir </path/to/data> --networkid <any-custom-network-id> --genesis </path/to/genesis.json> --bootnodes </path/to/bootnodes.json>
```
for a deeper explanation on this command, see the readme file under the **blockchain** directory